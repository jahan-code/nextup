...
export async function POST(
  ...
  try {
    ...
    if (shouldSkip) {
      ...
      const nextRoomStream =
        sorted.find((s) => s.streamId !== streamId) || null;
      ...
      console.log(
        `[Auto-Remove] Deleting skipped stream from queue: ${roomStream.id}`,
      );
      await prismaClient.roomStream
        ...
        .catch((err: any) => {
          console.log(
            `[Auto-Remove] Failed to delete skipped stream (may already be deleted): ${err.message}`,
          );
        });
      console.log(
        `[Auto-Remove] Successfully removed skipped stream from queue`,
      );
    }

    return successResponse({
      votes: votes.map((v: any) => v.userId),
      threshold,
      shouldSkip,
      nextStreamId,
    });
    ...
    return handleApiError(
      error,
      "POST /api/rooms/[id]/streams/[streamId]/skip",
    );
  }
}
